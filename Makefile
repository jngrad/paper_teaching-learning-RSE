...
objects := intro.pdf competencies.pdf institutionalized_education.pdf survey.pdf call_to_action.pdf ProjectTeamStructuresNew.pdf
...
%.pdf: %.md bibliography.bib contributors.yml preamble.sty
	...
	pandoc -V papersize:a4 -s --bibliography=bibliography.bib --biblatex -M date="`date "+%B %e, %Y"`" -V colorlinks=true --toc --from markdown+grid_tables -o "build/${@:.pdf=}.tex" "build/$<"
